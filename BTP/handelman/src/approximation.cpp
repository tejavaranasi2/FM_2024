     ...
     for(int itr=0;itr<iter_limit;++itr){
        ...
        try{
            for(int idx_solver=0;idx_solver<s_pos.size();++idx_solver){
                ...
                if(curr_box->box().substitute(s_pos[idx_solver],id_map,0)){
                    curr_box->box().curr_state_vec_()[idx_solver]=1;
                    lp_calls+=1;
                }
                else if(curr_box->box().substitute(s_neg[idx_solver],id_map,0)){
                    curr_box->box().curr_state_vec_()[idx_solver]=-1;
                    curr_box=frags->remove(curr_box);
                    found=0;
                    lp_calls+=1;
...
void fragmentation_intersection::run_threads(int iter_limit,double vol_limit,std::string FileName,trie_node* trie,int n_threads){
    std::vector<std::pair<double,double>> bounds;
    if(frags->HEAD!=NULL){
        bounds= frags->HEAD->box().coords_();
    }
...
bool bounding_box::make_smt_query(solver* s,std::vector<handelman::expr>e_check,formula* f){
  return s->smt_query(e_check,coords,f);
}
...
void fragmentation_formula::run_iters(formula* f,int iter_limit,double vol_limit,std::string FileName,trie_node* trie){
    //direclty runs the algorithm on this ll::
    int fnd=0;
    int lp_calls=0;
    std::vector<std::pair<double,double>> bounds;
    if(frags->HEAD!=NULL){
     ...
     for(int itr=0;itr<iter_limit;++itr){
        ...
        try{
            int res=f->solve(curr_box->box(),id_map,lp_calls);
            found=res+1;
